...
**Pending phases:** B = marketplace intelligence (empty-run via GPS, route-match, per-region supply/demand + Market Heat Score, 4-tier recommendations w/ acceptance%/wait); C = environmental metrics + mobile/shipper UI wiring + client estimator update; D = capture pricing signals + deterministic self-tuning (predictive models plug in later, NO LLM for price); E = reputation-based pricing; F = smart load bundling. Also: transparent breakdown + savings-vs-traditional surfaced in apps.
