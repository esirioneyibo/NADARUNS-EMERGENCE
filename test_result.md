...
agent_communication:
    ...
    - agent: "main"
      message: "FINNISH ROAD-FREIGHT PRICING (chargeable weight) implemented. Backend services/pricing.py now charges the WEIGHT component on the 'chargeable freight weight' = greatest of actual / volumetric (volume_m3 x 333) / pallet (pallets x 925) / loading-meter (loading_m x 1850), at a per-kg freight rate per vehicle. New request fields cargo_volume_m3, pallet_count, loading_meters on /api/shipper/quote and /api/shipper/shipments; new response fields freight_fee, chargeable_weight, chargeable_basis, actual_weight_kg. Frontend src/utils/pricing.ts mirrors this; shipper-create.tsx adds Pallets + Loading-meters inputs (Advanced section), computes volume from L×W×H dims, and the price breakdown shows 'Freight weight (X kg · basis)'. Localized EN+FI. NOTE FOR TESTING: Google Directions billing is OFF on the preview key, so /api/shipper/quote and /api/shipper/shipments will return 503 'routing configuration error' — this is EXPECTED in preview (user is enabling billing). The frontend shows an INSTANT LOCAL ESTIMATE (no Google needed), so the price card + freight breakdown render regardless. FRONTEND TEST FOCUS: login as demo shipper, create shipment, on the price/review step expand Advanced, enter pallets (e.g. 2) and/or dimensions, and confirm the breakdown line shows the chargeable freight weight and the total updates; verify EN and FI both render with no raw i18n keys. Do NOT treat the 503 on final create as a bug."

    - agent: "main"
      message: "PLATFORM ENHANCEMENTS (Tasks 1-3) implemented. BACKEND TEST FOCUS: (1) EMAIL INFRA (Brevo via httpx, provider=brevo configured in backend/.env). Transactional emails fire (fire-and-forget) + logged to email_logs collection on: driver register (/api/auth/driver-register & /api/driver/register -> welcome), shipper register (/api/auth/shipper-register & /api/shipper/register -> welcome), KYC approve/reject (/api/admin/.../kyc approve|reject -> driver_approved/rejected), order created (/api/shipper/shipments -> order_created), shipment lifecycle (accept/advance -> push_status_to_shipper sends driver_assigned + shipment_status), change-password (-> password_changed). Verify a welcome email row appears in GET /api/admin/email-logs after registering a driver. (2) AUTOMATED INVOICES & RECEIPTS: payment receipt auto-created+emailed on order capture (idempotent, db.receipts, doc_type=payment_receipt); withdrawal invoice on /api/wallet/withdraw (doc_type=withdrawal_invoice); payout receipt when admin marks withdrawal paid (doc_type=withdrawal_receipt); shipper invoice PDF emailed on /api/shipper/shipments/{id}/accept-invoice and on admin resend. NEW ENDPOINTS: GET /api/admin/receipts (admin), GET /api/receipts/{id}/pdf (owner/admin, token query or bearer), POST /api/admin/receipts/{id}/resend (admin), GET /api/shipper/receipts (shipper), GET /api/admin/email-logs (admin). (3) POP/POD VISIBILITY: shipper-tracking.tsx now shows a 'Proof of Pickup & Delivery' card with pickup_photo/delivery_photo (tap to fullscreen). Admin web Orders drawer shows POP/POD (Next.js, not in expo test scope). Creds in /app/memory/test_credentials.md (admin@nadaruns.com/admin123, demo.shipper@nadaruns.com/demo1234, demo.driver@nadaruns.com/demo1234). NOTE: Google Directions billing now enabled by user so /api/shipper/shipments should work. Do NOT modify .env URLs. Test backend Tasks 1-2 thoroughly; for Task 3 frontend, verify the POP/POD card renders when an order has photos (you may attach via driver POST /api/orders/{id}/photo)."

    - agent: "main"
      message: "TASK 4 — Driver Onboarding Redesign implemented. BACKEND: DriverRegistration model extended with account_type ('individual'|'fleet'), license_class, company_name, business_id, company_phone, company_email, company_address. POST /api/driver/register now: validates fleet requires company_name (400 if missing); for fleet creates a Company (owner_driver_id=driver), inits company_wallet, and sets driver.company_id + company_role='owner'; stores license_class on driver doc. Verified via curl + DB inspection (company + wallet created, driver linked). FRONTEND (app/onboarding.tsx): wizard refactored to dynamic steps — Step1 account type (Individual vs Fleet, testIDs account-individual/account-fleet), then Name, Contact, [Company step ONLY for fleet: testIDs input-company-name/input-business-id/input-company-email/input-company-address], Vehicle (now also licence class chips testID license-<X> + plate input-plate), Review. Info tooltips via testID tooltip-<field> (Alert popups). Individual path = 5 steps, Fleet = 6 steps. api.registerDriver type extended. PLEASE TEST: (1) Individual full registration completes & lands in app (use unique email). (2) Fleet registration with company name completes; (3) fleet without company name blocks Continue on company step. Do NOT re-test Tasks 1-3 (email/receipts/POP-POD) — already green in iter37. Creds in /app/memory/test_credentials.md."
