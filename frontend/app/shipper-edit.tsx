...
export default function ShipperEditScreen() {
  ...
  const flash = (msg: string, ok = true) => { setBanner({ msg, ok }); setTimeout(() => setBanner(null), 2600); };

  const load = useCallback(async () => {
    try {
      const d = await api.getShipper();
      ...
      setAvatar(d.avatar || "");
      setLoaded(true);
    } catch (e) { console.warn("shipper-edit load failed", e); setLoaded(true); }
  }, []);
  ...
  const pickAvatar = async () => {
    ...
    try {
      ...
      if (!granted) { flash("Photo access denied. Enable it in Settings.", false); if (Platform.OS !== "web") Linking.openSettings().catch(() => {}); return; }
    ...
    } catch (e) { flash("Could not open gallery.", false); }
  };

  const save = async () => {
    if (!companyName.trim()) { flash("Company name can't be empty.", false); return; }
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(() => {});
    setSaving(true);
    try {
      await api.updateShipper({ company_name: companyName.trim(), contact_name: contactName.trim(), phone: phone.trim(), address: address.trim(), tax_id: taxId.trim(), avatar });
      flash("Profile updated");
      setTimeout(() => router.back(), 700);
    } catch { flash("Failed to save. Try again.", false); }
        ...
        >
          <View style={styles.avatarWrap}>
            {avatar ? (
              ...
              <View style={[styles.avatar, styles.avatarPlaceholder]}><Ionicons name="business" size={40} color="#fff" /></View>
            )}
            <TouchableOpacity style={[styles.avatarBtn, shadows.md]} onPress={pickAvatar}>
              <Ionicons name="camera" size={18} color="#fff" />
            </TouchableOpacity>
          </View>
          <Text style={styles.avatarHint}>Tap the camera to change your logo</Text>

          <Text style={styles.sectionTitle}>COMPANY INFORMATION</Text>
          <View style={[styles.card, shadows.sm]}>
            <Field label="Company name" icon="business-outline" value={companyName} onChangeText={setCompanyName} theme={theme} />
            <Divider theme={theme} />
            <Field label="Contact name" icon="person-outline" value={contactName} onChangeText={setContactName} theme={theme} />
            <Divider theme={theme} />
            <Field label="Phone" icon="call-outline" value={phone} onChangeText={setPhone} keyboardType="phone-pad" theme={theme} />
            <Divider theme={theme} />
            <Field label="Business address" icon="location-outline" value={address} onChangeText={setAddress} theme={theme} />
            <Divider theme={theme} />
            <Field label="Tax ID / VAT" icon="document-text-outline" value={taxId} onChangeText={setTaxId} autoCapitalize="characters" theme={theme} />
          </View>
...
function Divider({ theme }: any) { return <View style={{ height: 1, backgroundColor: theme.border, marginLeft: 32 }} />; }
